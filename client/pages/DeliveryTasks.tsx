...
import {
  ...
  ClipboardList, FileText, RotateCcw, Settings as SettingsIcon
} from 'lucide-react';
...
const mockDeliveryTasks: DeliveryTask[] = [
  {
    ...
    deliveryAddress: 'حي عرفات، شارع الجمهورية، بج��نب محطة الوقود',
    deliveryCost: 500,
    status: 'in_progress',
    courierId: 'courier1',
          ...
          <SheetContent side="right" className="w-full sm:max-w-2xl max-h-screen overflow-y-auto">
            <SheetHeader>
              <SheetTitle className="flex items-center gap-2">
                <SettingsIcon size={18} />
                إعدادات مهام التوصيل
              </SheetTitle>
            </SheetHeader>
            <DeliverySettingsPanel onClose={() => setShowSettings(false)} />
          </SheetContent>
        </Sheet>
...
const DeliverySettingsPanel: React.FC<{ onClose?: () => void }> = ({ onClose }) => {
  const { settings, update } = useSettings();
  const [zones, setZones] = useState(settings.delivery.zones || []);
  const [zoneForm, setZoneForm] = useState<{ id: string; name: string; fee: number }>({ id: '', name: '', fee: 0 });
  const [feeRules, setFeeRules] = useState(settings.delivery.feeRules || []);
  const [feeForm, setFeeForm] = useState<{ id: string; zoneId?: string; minWeightKg?: number; maxWeightKg?: number; minOrderAmountMRU?: number; feeMRU: number }>({ id:'', zoneId:'', minWeightKg:0, maxWeightKg:0, minOrderAmountMRU:0, feeMRU:0 });
  const [regions, setRegions] = useState(settings.delivery.regions || []);
  const [regionForm, setRegionForm] = useState<{ id: string; name: string; enabled: boolean; specialFeeMRU?: number }>({ id:'', name:'', enabled:true, specialFeeMRU:0 });
  const [drivers, setDrivers] = useState(settings.delivery.drivers || []);
  const [driverForm, setDriverForm] = useState<{ id: string; name: string; phone?: string; commissionPercent: number; active: boolean }>({ id:'', name:'', phone:'', commissionPercent: settings.delivery.courierProfitPercent || 20, active:true });
  const [baseMinutes, setBaseMinutes] = useState<number>(settings.delivery.estimation?.baseMinutes || 20);
  const [perKmMinutes, setPerKmMinutes] = useState<number>(settings.delivery.estimation?.perKmMinutes || 3);
  const [holidays, setHolidays] = useState<string[]>(settings.delivery.holidays || []);
  const [policy, setPolicy] = useState({ ...(settings.delivery.policies || { redeliveryAttempts: 2, notifySMS: true, notifyWhatsApp: false }) });

  useEffect(()=> setZones(settings.delivery.zones || []), [settings.delivery.zones]);
  useEffect(()=> setFeeRules(settings.delivery.feeRules || []), [settings.delivery.feeRules]);
  useEffect(()=> setRegions(settings.delivery.regions || []), [settings.delivery.regions]);
  useEffect(()=> setDrivers(settings.delivery.drivers || []), [settings.delivery.drivers]);

  const saveZone = async () => {
    if (!zoneForm.name.trim()) return;
    const id = zoneForm.id && zoneForm.id.trim()!=='' ? zoneForm.id : 'zone_' + Date.now();
    const item = { ...zoneForm, id, fee: Math.max(0, Math.round(Number(zoneForm.fee)||0)) };
    const next = zones.some(z=> z.id===id) ? zones.map(z=> z.id===id? item: z) : [item, ...zones];
    setZones(next);
    await update({ delivery: { ...settings.delivery, zones: next } });
    setZoneForm({ id:'', name:'', fee:0 });
  };
  ...
  const saveFeeRule = async () => {
    const id = feeForm.id && feeForm.id.trim()!=='' ? feeForm.id : 'fee_' + Date.now();
    const item = { ...feeForm, id, feeMRU: Math.max(0, Math.round(Number(feeForm.feeMRU)||0)) };
    const next = feeRules.some(r=> r.id===id) ? feeRules.map(r=> r.id===id? item: r) : [item, ...feeRules];
    setFeeRules(next);
    await update({ delivery: { ...settings.delivery, feeRules: next } });
    setFeeForm({ id:'', zoneId:'', minWeightKg:0, maxWeightKg:0, minOrderAmountMRU:0, feeMRU:0 });
  };
  const removeFeeRule = async (id: string) => {
    const next = feeRules.filter(r=> r.id !== id);
    setFeeRules(next);
    await update({ delivery: { ...settings.delivery, feeRules: next } });
  };
  ...
  const saveRegion = async () => {
    ...
    const id = regionForm.id && regionForm.id.trim()!=='' ? regionForm.id : 'reg_' + Date.now();
    const item = { ...regionForm, id, specialFeeMRU: Math.max(0, Math.round(Number(regionForm.specialFeeMRU||0))) } as any;
    const next = regions.some(r=> r.id===id) ? regions.map(r=> r.id===id? item: r) : [item, ...regions];
    setRegions(next);
    await update({ delivery: { ...settings.delivery, regions: next } });
    setRegionForm({ id:'', name:'', enabled:true, specialFeeMRU:0 });
  };
  ...
  const saveDriver = async () => {
    ...
    const id = driverForm.id && driverForm.id.trim()!=='' ? driverForm.id : 'drv_' + Date.now();
    const item = { ...driverForm, id, commissionPercent: Math.max(0, Math.min(100, Math.round(Number(driverForm.commissionPercent)||0))) };
    const next = drivers.some(d=> d.id===id) ? drivers.map(d=> d.id===id? item: d) : [item, ...drivers];
    setDrivers(next);
    await update({ delivery: { ...settings.delivery, drivers: next } });
    setDriverForm({ id:'', name:'', phone:'', commissionPercent: settings.delivery.courierProfitPercent || 20, active:true });
  };
  ...
  const saveSchedule = async () => {
    await update({ delivery: { ...settings.delivery, estimation: { baseMinutes, perKmMinutes }, holidays } });
  };
  const savePolicies = async () => {
    await update({ delivery: { ...settings.delivery, policies: policy } });
    onClose?.();
  };
    ...
    <div className="mt-4">
      <Tabs defaultValue="fees" className="w-full">
        <TabsList className="flex gap-2 p-2 bg-gray-100 dark:bg-gray-800 rounded-lg overflow-x-auto whitespace-nowrap sticky top-0 z-10">
          <TabsTrigger value="fees">رسوم التوصيل</TabsTrigger>
          <TabsTrigger value="regions">المناطق/التغطية</TabsTrigger>
          <TabsTrigger value="drivers">السائقين</TabsTrigger>
          <TabsTrigger value="schedule">الجدولة والتوقيت</TabsTrigger>
          <TabsTrigger value="policies">سياسات التوصيل</TabsTrigger>
        </TabsList>

        <TabsContent value="fees" className="mt-4">
          <Card><CardContent className="p-4 sm:p-6 space-y-4">
            <div className="p-3 rounded border">
              <div className="font-medium arabic-safe mb-2">المناطق والرسوم الأساسية</div>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                <Input placeholder="اسم المنطقة" value={zoneForm.name} onChange={(e)=> setZoneForm({ ...zoneForm, name: e.target.value })} />
                <Input type="number" placeholder="رسوم (MRU)" value={zoneForm.fee} onChange={(e)=> setZoneForm({ ...zoneForm, fee: parseInt(e.target.value)||0 })} />
                <Button onClick={saveZone}>حفظ</Button>
                <Button variant="outline" onClick={()=> setZoneForm({ id:'', name:'', fee:0 })}>جديد</Button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-2">
                {zones.map(z=> (
                  <div key={z.id} className="p-2 rounded border flex items-center justify-between text-sm">
                    <div>{z.name} • {formatCurrencyMRU(z.fee)} MRU</div>
                    <div className="flex items-center gap-2">
                      <Button size="sm" variant="outline" onClick={()=> setZoneForm(z)}>تعديل</Button>
                      <Button size="sm" variant="destructive" onClick={()=> removeZone(z.id)}>حذف</Button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="p-3 rounded border">
              <div className="font-medium arabic-safe mb-2">قواعد إضافية حسب الوزن/القيمة</div>
              <div className="grid grid-cols-1 md:grid-cols-6 gap-2">
                <select value={feeForm.zoneId} onChange={(e)=> setFeeForm({ ...feeForm, zoneId: e.target.value })} className="px-3 py-2 border rounded">
                  <option value="">كل المناطق</option>
                  {zones.map(z=> (<option key={z.id} value={z.id}>{z.name}</option>))}
                </select>
                <Input type="number" placeholder="وزن أدنى (كجم)" value={feeForm.minWeightKg as any} onChange={(e)=> setFeeForm({ ...feeForm, minWeightKg: parseFloat(e.target.value)||0 })} />
                <Input type="number" placeholder="وزن أقصى (كجم)" value={feeForm.maxWeightKg as any} onChange={(e)=> setFeeForm({ ...feeForm, maxWeightKg: parseFloat(e.target.value)||0 })} />
                <Input type="number" placeholder="حد أدنى لقيمة الطلب" value={feeForm.minOrderAmountMRU as any} onChange={(e)=> setFeeForm({ ...feeForm, minOrderAmountMRU: parseInt(e.target.value)||0 })} />
                <Input type="number" placeholder="الرسوم (MRU)" value={feeForm.feeMRU as any} onChange={(e)=> setFeeForm({ ...feeForm, feeMRU: parseInt(e.target.value)||0 })} />
                <Button onClick={saveFeeRule}>إضافة/حفظ</Button>
              </div>
              <div className="space-y-2 mt-2">
                {feeRules.map(r=> (
                  <div key={r.id} className="p-2 rounded border flex items-center justify-between text-xs">
                    <div>
                      {r.zoneId ? (zones.find(z=> z.id===r.zoneId)?.name || r.zoneId) : 'كل المناطق'} • وزن: {r.minWeightKg||0}-{r.maxWeightKg||0} • حد الطلب: {r.minOrderAmountMRU||0} • رسوم: {formatCurrencyMRU(r.feeMRU)}
                    </div>
                    <div className="flex gap-2">
                      <Button size="sm" variant="outline" onClick={()=> setFeeForm(r as any)}>تعديل</Button>
                      <Button size="sm" variant="destructive" onClick={()=> removeFeeRule(r.id)}>حذف</Button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <TabsContent value="regions" className="mt-4">
          <Card><CardContent className="p-4 sm:p-6 space-y-3">
            <div className="grid grid-cols-1 md:grid-cols-5 gap-2">
              <Input placeholder="اسم المدينة/المنطقة" value={regionForm.name} onChange={(e)=> setRegionForm({ ...regionForm, name: e.target.value })} />
              <Input type="number" placeholder="رسوم خاصة (MRU)" value={regionForm.specialFeeMRU as any} onChange={(e)=> setRegionForm({ ...regionForm, specialFeeMRU: parseInt(e.target.value)||0 })} />
              <label className="inline-flex items-center gap-2 text-sm arabic-safe"><input type="checkbox" checked={!!regionForm.enabled} onChange={(e)=> setRegionForm({ ...regionForm, enabled: e.target.checked })} /> مفعلة</label>
              <Button onClick={saveRegion}>حفظ</Button>
              <Button variant="outline" onClick={()=> setRegionForm({ id:'', name:'', enabled:true, specialFeeMRU:0 })}>جديد</Button>
            </div>
            <div className="space-y-2">
              {regions.map(r=> (
                <div key={r.id} className="p-2 rounded border flex items-center justify-between text-sm">
                  <div>{r.name} • {r.enabled ? 'مفعلة' : 'معطلة'} {r.specialFeeMRU ? `• ${formatCurrencyMRU(r.specialFeeMRU)} MRU` : ''}</div>
                  <div className="flex items-center gap-2">
                    <Button size="sm" variant="outline" onClick={()=> setRegionForm(r)}>تعديل</Button>
                    <Button size="sm" variant="destructive" onClick={()=> removeRegion(r.id)}>حذف</Button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <TabsContent value="drivers" className="mt-4">
          <Card><CardContent className="p-4 sm:p-6 space-y-3">
            <div className="grid grid-cols-1 md:grid-cols-6 gap-2">
              <Input placeholder="اسم السائق" value={driverForm.name} onChange={(e)=> setDriverForm({ ...driverForm, name: e.target.value })} />
              <Input placeholder="الهاتف" value={driverForm.phone||''} onChange={(e)=> setDriverForm({ ...driverForm, phone: e.target.value })} />
              <Input type="number" placeholder="نسبة العمولة (%)" value={driverForm.commissionPercent as any} onChange={(e)=> setDriverForm({ ...driverForm, commissionPercent: parseInt(e.target.value)||0 })} />
              <label className="inline-flex items-center gap-2 text-sm arabic-safe"><input type="checkbox" checked={!!driverForm.active} onChange={(e)=> setDriverForm({ ...driverForm, active: e.target.checked })} /> نشط</label>
              <Button onClick={saveDriver}>حفظ</Button>
              <Button variant="outline" onClick={()=> setDriverForm({ id:'', name:'', phone:'', commissionPercent: settings.delivery.courierProfitPercent || 20, active:true })}>جديد</Button>
            </div>
            <div className="space-y-2">
              {drivers.map(d=> (
                <div key={d.id} className="p-2 rounded border flex items-center justify-between text-sm">
                  <div>{d.name} • {d.phone||'-'} • {d.commissionPercent}% • {d.active ? 'نشط' : 'معطل'}</div>
                  <div className="flex items-center gap-2">
                    <Button size="sm" variant="outline" onClick={()=> setDriverForm(d)}>تعديل</Button>
                    <Button size="sm" variant="destructive" onClick={()=> removeDriver(d.id)}>حذف</Button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <TabsContent value="schedule" className="mt-4">
          <Card><CardContent className="p-4 sm:p-6 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-medium text-gray-700 arabic-safe mb-1">زمن أساسي للتوصيل (دقيقة)</label>
                <NumericInput value={String(baseMinutes)} onChange={(v)=> setBaseMinutes(Math.max(0, Math.round(Number(v)||0)))} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 arabic-safe mb-1">دقائق لكل كم</label>
                <NumericInput value={String(perKmMinutes)} onChange={(v)=> setPerKmMinutes(Math.max(0, Math.round(Number(v)||0)))} />
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 arabic-safe mb-1">أيام العطلة</label>
              <div className="flex flex-wrap gap-2">
                {holidays.map((h, idx)=>(
                  <div key={idx} className="px-2 py-1 rounded border text-xs flex items-center gap-2">
                    <span>{h}</span>
                    <Button size="sm" variant="ghost" onClick={()=> setHolidays(holidays.filter(x=> x!==h))}>حذف</Button>
                  </div>
                ))}
              </div>
              <div className="mt-2 grid grid-cols-1 md:grid-cols-3 gap-2">
                <Input type="date" onChange={(e)=> e.target.value && setHolidays(Array.from(new Set([...(holidays||[]), e.target.value])))} />
                <div className="md:col-span-2 flex items-center"><span className="text-xs text-gray-500 arabic-safe">أضف تواريخ العطلة</span></div>
              </div>
            </div>
            <div className="flex justify-end gap-2"><Button variant="outline" onClick={()=> { setBaseMinutes(settings.delivery.estimation?.baseMinutes||20); setPerKmMinutes(settings.delivery.estimation?.perKmMinutes||3); setHolidays(settings.delivery.holidays||[]); }}>إلغاء</Button><Button className="bg-blue-600 hover:bg-blue-700" onClick={saveSchedule}>حفظ</Button></div>
          </CardContent></Card>
        </TabsContent>
        ...
        <TabsContent value="policies" className="mt-4">
          <Card><CardContent className="p-4 sm:p-6 space-y-3">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-medium text-gray-700 arabic-safe mb-1">عدد محاولات إعادة التوصيل</label>
                <NumericInput value={String(policy.redeliveryAttempts || 0)} onChange={(v)=> setPolicy({ ...policy, redeliveryAttempts: Math.max(0, Math.round(Number(v)||0)) })} />
              </div>
            </div>
            <div className="space-y-2">
              <label className="inline-flex items-center gap-2 text-sm arabic-safe"><input type="checkbox" checked={!!policy.notifySMS} onChange={(e)=> setPolicy({ ...policy, notifySMS: e.target.checked })} /> إشعارات SMS</label>
              <label className="inline-flex items-center gap-2 text-sm arabic-safe"><input type="checkbox" checked={!!policy.notifyWhatsApp} onChange={(e)=> setPolicy({ ...policy, notifyWhatsApp: e.target.checked })} /> إشعارات WhatsApp</label>
            </div>
            <div className="flex justify-end"><Button className="bg-blue-600 hover:bg-blue-700" onClick={savePolicies}>حفظ الإعدادات</Button></div>
          </CardContent></Card>
        </TabsContent>
      </Tabs>
    </div>
