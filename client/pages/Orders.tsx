...
import {
  ...
  Users, Store, Link, Hash, Weight, X, Settings
} from 'lucide-react';
...
import { calculateCommissionAmount, applyDiscount } from '@shared/commerce';
...
export interface WeightCalculation {
  ...
  totalShippingCost: number; // إجمالي تك��فة الشحن
...
const mockOrders: Order[] = [
  ...
  },
  {
    ...
    productLinks: [
      ...
      { url: 'https://ebay.com/item3', quantity: 1, notes: 'مع الصندوق الأصل��' }
    ],
    productCount: 4,
    originalPrice: 280,
    originalCurrency: 'USD',
  ...
  },
  {
    ...
    commission: 30000,
    discount: 10000,
    status: 'arrived', // وصل لل��خزن لكن مدفوع جزئياً
    weight: 2.8,
    ...
    weightCalculation: {
      ...
      shippingCost: 450, // شحن سري�� من دبي
...
const Orders: React.FC = () => {
  ...
  const customerRates = settings.ordersCustomerShipping || { air_standard_perKg: DEFAULT_SHIPPING_SETTINGS.weightPricePerKg, air_express_perKg: DEFAULT_SHIPPING_SETTINGS.weightPricePerKg * 1.8, sea_perKg: 600, land_perKg: 900, localDeliveryMRU: DEFAULT_SHIPPING_SETTINGS.localDeliveryPrice };
    ...
    const off = eventBus.on('settings.currencies.changed', () => {
      try {
        ...
        setOrders(prev => prev.map(o => {
          ...
          const rate = Number((settings.currencies.rates as any)[o.originalCurrency]) || 1;
          const converted = Math.round(o.originalPrice * rate);
          const commission = calculateCommissionAmount(
            converted,
            settings.ordersInvoices.commissionPolicies || [],
            o.store.id,
            settings.ordersInvoices.defaultCommissionPercent
          );
          const fixedDiscountMRU = o.discountType === 'fixed' ? Math.round((o.discountValue || 0) * rate) : undefined;
          const discount = applyDiscount(
            converted,
            o.discountType,
            o.discountType === 'fixed' ? fixedDiscountMRU : (o.discountValue || 0)
          );
  ...
  // ت��بيق الفلاتر والترتيب مع الترتيب حسب اأحدث أولاً كفتراضي
  const sortedOrders = React.useMemo(() => {
    ...
    // فلتر الحالة مع مرا��اة الطلبات المدفوع جزئياً
    if (statusFilter !== 'all') {
      filtered = filtered.filter(order => {
  ...
  // الحصول على قائمة المتاجر ا��متاحة
      ...
      <CardHeader className="pb-3 bg-gradient-to-r from-gray-50 to-white dark:from-gray-800 dark:to-gray-700 rounded-t-lg p-4 sm:p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {!order.trackingNumber && (
              <AlertCircle size={18} className="text-red-500 animate-pulse" aria-label="الطلب بدون رقم تتبع" />
            )}
          </div>
        ...
        <div className="pt-3 space-y-2">
          ...
          {['arrived', 'weight_paid', 'in_delivery', 'delivered'].includes(order.status) && (
            <Button
              ...
              title={order.weightCalculation ? "تعديل حساب الوزن والشحن" : "حساب الوزن والشحن"}
            >
          ...
          {order.weightCalculation && (
            <div className="p-3 bg-green-50 rounded-lg border border-green-200">
              <div className="text-xs text-green-700 mb-2 arabic-safe">تفاصيل الشحن:</div>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 lg:p-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-2 w-full lg:w-auto">
            ...
            <Button
              ...
              className="w-full sm:w-auto h-12 sm:h-10 arabic-safe no-text-break"
      ...
      <Sheet open={showSettings} onOpenChange={setShowSettings}>
        <SheetContent side="right" className="w-full sm:max-w-2xl max-h-screen overflow-y-auto">
          ...
          <OrdersSettingsPanel onClose={() => setShowSettings(false)} />
        </SheetContent>
      </Sheet>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-red-700 dark:text-red-300 mt-1 arabic-safe">
                  لديك {ordersWithoutTracking.length} طلب{ordersWithoutTracking.length > 1 ? 'ات' : ''} بدون رقم تتبع. يرجى إضافة أرقام التتبع لمتابعة حالة الشحن.
                </p>
              </div>
              <Button
                size="sm"
        ...
        {/* زر إظهار/إخفاء الفلاتر السريعة */}
        <div className="mb-4">
          <Button
            variant="outline"
          ...
          >
            <Filter size={16} />
            {showQuickFilters ? 'إخفاء الفلاتر ال��ريعة' : 'إظهار الفلاتر السريعة'}
              ...
              <div className="border-t pt-4 mt-4">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  >
                    ...
                    <option value="tracked">مع رقم تتبع</option>
                ...
                <div className="mt-4 bg-teal-50 p-4 rounded-lg border border-teal-200">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h4 className="font-medium text-teal-800 arabic-safe">فلاتر ذكية للمخزن</h4>
                  </div>
                    ...
                    >
                      <td className="px-4 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          {!order.trackingNumber && (
                            <AlertCircle size={14} className="text-red-500 mr-1 animate-pulse" aria-label="الطلب بدون رقم تتبع" />
                          )}
                        </div>
                      </td>
          ...
          <Card>
            <CardContent className="text-center py-12">
              ...
              <p className="text-gray-600 dark:text-gray-400">
                لم يتم العثور على أي طلبات تطابق البحث أو الفلاتر المحددة.
              </p>
            </CardContent>
          </Card>
        )}
        ...
        {weightModalOrder && (
          <WeightModal
            ...
            shippingSettings={{
              weightPricePerKg: customerRates.air_standard_perKg,
              normalShippingFromDubai: 0,
              expressShippingFromDubai: 0,
              localDeliveryPrice: settings.delivery?.insideNKCPrice || 0,
              // extended for per-type MRU/kg
              ...( { airStandardPerKg: customerRates.air_standard_perKg, airExpressPerKg: customerRates.air_express_perKg } as any )
            } as any}
          />
        )}
      </div>
...
const OrdersSettingsPanel: React.FC<{ onClose?: () => void }> = ({ onClose }) => {
  const { settings, update } = useSettings();
  const [rates, setRates] = useState<Record<string, number>>({ ...settings.currencies.rates });
  const [weightPreview, setWeightPreview] = useState<string>('');
  const [weightType, setWeightType] = useState<'air_express'|'air_standard'|'sea'|'land'>('air_standard');

  const convertFromMRU = (mru: number, to: string) => {
    const rate = (settings.currencies.rates as any)[to];
    if (!rate || rate <= 0) return `${formatCurrencyMRU(mru)} MRU`;
    const val = mru / rate;
    return `${new Intl.NumberFormat('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(val)} ${to}`;
  };

  const previewCost = () => {
    const w = Number(weightPreview) || 0;
    const ratePerKg = weightType==='air_express' ? (settings.ordersCustomerShipping?.air_express_perKg||0) : weightType==='sea'? (settings.ordersCustomerShipping?.sea_perKg||0) : weightType==='land'? (settings.ordersCustomerShipping?.land_perKg||0) : (settings.ordersCustomerShipping?.air_standard_perKg||0);
    const minC = settings.ordersCustomerShipping?.minChargePerItem || 0;
    const cost = Math.max(minC, Math.round(w * ratePerKg));
    const display = settings.ordersPreferences?.displayCurrency || 'MRU';
    return {
      mru: formatCurrencyMRU(cost),
      display: display==='MRU' ? formatCurrencyMRU(cost) + ' MRU' : convertFromMRU(cost, display)
    };
  };
    ...
    <div className="mt-4">
      <Tabs defaultValue="currencies" className="w-full">
        <TabsList className="flex gap-2 p-2 bg-gray-100 dark:bg-gray-800 rounded-lg overflow-x-auto whitespace-nowrap sticky top-0 z-10">
          ...
          <TabsTrigger value="customerShipping">أسعار الوزن للزبون</TabsTrigger>
          <TabsTrigger value="policies">سياسات الطلبات</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="currencies" className="mt-4">
          <Card><CardContent className="p-4 sm:p-6 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 arabic-safe mb-1">العملة الأساسية</label>
                <select value={settings.currencies.base} onChange={(e)=> update({ currencies: { ...settings.currencies, base: e.target.value as any } })} className="w-full px-3 py-2 border rounded">
                  {Object.keys(settings.currencies.rates).concat(['MRU','USD','AED','EUR']).filter((v,i,a)=> a.indexOf(v)===i).map(c=> (
                    <option key={c} value={c as any}>{c}</option>
                  ))}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 arabic-safe mb-1">آخر تحديث</label>
                <Input value={settings.currencies.lastUpdated ? new Date(settings.currencies.lastUpdated).toLocaleString() : ''} disabled />
              </div>
              <div className="flex items-end">
                <Button className="w-full sm:w-auto" onClick={async ()=>{
                  try {
                    ...
                    if (data && data.rates) {
                      const usdToMRU = data.rates['MRU'];
                      const nextRates: Record<string, number> = { ...settings.currencies.rates };
                      Object.keys(nextRates).forEach(k => { if (k !== 'MRU' && data.rates[k]) { nextRates[k] = 1 / (data.rates[k] / usdToMRU); } });
                      await update({ currencies: { ...settings.currencies, rates: nextRates, lastUpdated: new Date().toISOString(), source: 'API' } });
                    }
                  } catch {}
                }}>تحديث تلقائي من API</Button>
              </div>
            </div>

            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <div className="font-medium arabic-safe">أسعار الصرف مقابل MRU</div>
                <Button size="sm" onClick={()=>{
                  const code = prompt('أدخل رمز العملة (مثال USD)');
                  if (!code) return;
                  if ((settings.currencies.rates as any)[code]) return;
                  setRates({ ...rates, [code]: 0 });
                }}>إضافة عملة</Button>
              </div>
              ...
              <div className="flex justify-end gap-2">
                <Button variant="outline" onClick={()=> setRates(settings.currencies.rates as any)}>إلغاء</Button>
                <Button className="bg-blue-600 hover:bg-blue-700" onClick={() => update({ currencies: { ...settings.currencies, rates, lastUpdated: new Date().toISOString(), source: 'Manual' } })}>حفظ</Button>
              </div>
            </div>
          </CardContent></Card>
        </TabsContent>
        ...
        <TabsContent value="commission" className="mt-4">
          <Card><CardContent className="p-4 sm:p-6 space-y-3">
            <div>
              ...
              <NumericInput value={String(settings.ordersPreferences?.platformFeePercent || 0)} onChange={(v)=> update({ ordersPreferences: { ...(settings.ordersPreferences||{}), platformFeePercent: Math.max(0, Math.min(100, Number(v)||0)) } })} />
            </div>
            <div className="p-3 rounded border">
              <div className="font-medium arabic-safe mb-2">عمولات خاصة</div>
              <div className="text-xs text-gray-600 arabic-safe mb-2">استخدم سياسات العمولة حسب المتجر من مركز الإعدادات إن لزم.</div>
            </div>
          </CardContent></Card>
        </TabsContent>
        ...
        <TabsContent value="warranty" className="mt-4">
          <Card><CardContent className="p-4 sm:p-6 space-y-3">
            <div>
              ...
              <NumericInput value={String(settings.ordersWarranty?.defaultDays || 0)} onChange={(v)=> update({ ordersWarranty: { ...(settings.ordersWarranty||{}), defaultDays: Math.max(0, Math.round(Number(v)||0) ) } })} />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 arabic-safe mb-1">سياسة الضمان</label>
              <Input value={settings.ordersWarranty?.guidanceText || ''} onChange={(e)=> update({ ordersWarranty: { ...(settings.ordersWarranty||{}), guidanceText: e.target.value } })} />
            </div>
            <WarrantyByCategoryEditor />
          </CardContent></Card>
        </TabsContent>

        <TabsContent value="customerShipping" className="mt-4">
          <Card><CardContent className="p-4 sm:p-6 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {(['air_standard','air_express','sea','land'] as const).map(k => (
                <div key={k}>
                  <label className="block text-sm font-medium text-gray-700 arabic-safe mb-1">{k} (MRU/كجم)</label>
                  <NumericInput value={String((settings.ordersCustomerShipping as any)[`${k}_perKg`] || 0)} onChange={(v)=> update({ ordersCustomerShipping: { ...settings.ordersCustomerShipping, [`${k}_perKg`]: Math.max(0, Math.round(Number(v)||0)) } as any })} />
                </div>
              ))}
              <div>
                <label className="block text-sm font-medium text-gray-700 arabic-safe mb-1">حد أدنى/عنصر (MRU)</label>
                <NumericInput value={String(settings.ordersCustomerShipping?.minChargePerItem || 0)} onChange={(v)=> update({ ordersCustomerShipping: { ...settings.ordersCustomerShipping, minChargePerItem: Math.max(0, Math.round(Number(v)||0)) } })} />
              </div>
            </div>
            <div className="p-3 rounded border bg-gray-50">
              <div className="font-medium arabic-safe mb-2">معاينة التكلفة</div>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-2 items-end">
                <div>
                  <label className="block text-xs text-gray-600 arabic-safe mb-1">الوزن (كجم)</label>
                  <NumericInput value={weightPreview} onChange={setWeightPreview} />
                </div>
                <div>
                  <label className="block text-xs text-gray-600 arabic-safe mb-1">النوع</label>
                  <select value={weightType} onChange={(e)=> setWeightType(e.target.value as any)} className="w-full px-3 py-2 border rounded">
                ...
                <div className="text-sm">
                  <div className="text-gray-600 arabic-safe">MRU</div>
                  <div className="font-medium">{previewCost().mru}</div>
                </div>
                <div className="text-sm">
                  <div className="text-gray-600 arabic-safe">{settings.ordersPreferences?.displayCurrency || 'MRU'}</div>
                  <div className="font-medium">{previewCost().display}</div>
                </div>
              </div>
            </div>
          </CardContent></Card>
        ...
        <TabsContent value="policies" className="mt-4">
          <Card><CardContent className="p-4 sm:p-6 space-y-3">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-medium text-gray-700 arabic-safe mb-1">الحد الأقصى للمنتجات في الطلب</label>
                <NumericInput value={String(settings.ordersPolicies?.maxItemsPerOrder || 0)} onChange={(v)=> update({ ordersPolicies: { ...(settings.ordersPolicies||{}), maxItemsPerOrder: Math.max(0, Math.round(Number(v)||0)) } })} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 arabic-safe mb-1">مدة المعالجة الافتراضية (يوم)</label>
                <NumericInput value={String(settings.ordersPolicies?.defaultProcessingDays || 0)} onChange={(v)=> update({ ordersPolicies: { ...(settings.ordersPolicies||{}), defaultProcessingDays: Math.max(0, Math.round(Number(v)||0)) } })} />
              </div>
            </div>
            <div className="space-y-2">
              <label className="inline-flex items-center gap-2 text-sm arabic-safe"><input type="checkbox" checked={!!settings.ordersPolicies?.notifySMS} onChange={(e)=> update({ ordersPolicies: { ...(settings.ordersPolicies||{}), notifySMS: e.target.checked } })} /> إشعارات SMS</label>
              <label className="inline-flex items-center gap-2 text-sm arabic-safe"><input type="checkbox" checked={!!settings.ordersPolicies?.notifyEmail} onChange={(e)=> update({ ordersPolicies: { ...(settings.ordersPolicies||{}), notifyEmail: e.target.checked } })} /> إشعارات Email</label>
            </div>
            <div className="flex justify-end"><Button className="bg-blue-600 hover:bg-blue-700" onClick={()=> onClose?.()}>حفظ الإعدادات</Button></div>
          </CardContent></Card>
        </TabsContent>
      </Tabs>
    </div>
...
const DiscountsEditor: React.FC = () => {
  const { settings, update } = useSettings();
  const [codes, setCodes] = useState(settings.ordersDiscounts?.codes || []);
  const [form, setForm] = useState<{ id: string; code: string; type: 'percentage'|'fixed'; value: number; validFrom?: string; validTo?: string; enabled: boolean; targetCategory?: string }>({ id: '', code: '', type: 'percentage', value: 0, validFrom: '', validTo: '', enabled: true, targetCategory: '' });

  useEffect(()=> setCodes(settings.ordersDiscounts?.codes || []), [settings.ordersDiscounts]);

  const save = () => {
    const id = form.id && form.id.trim()!=='' ? form.id : 'dc_' + Date.now();
    const item = { ...form, id, value: Math.max(0, Math.round(Number(form.value)||0)) };
    const next = codes.some(c=> c.id === id) ? codes.map(c=> c.id===id? item: c) : [item, ...codes];
    setCodes(next);
    update({ ordersDiscounts: { ...(settings.ordersDiscounts||{}), codes: next } });
    setForm({ id: '', code: '', type: 'percentage', value: 0, validFrom: '', validTo: '', enabled: true, targetCategory: '' });
  };
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-1 md:grid-cols-6 gap-2">
        ...
        <select value={form.type} onChange={(e)=> setForm({ ...form, type: e.target.value as any })} className="px-3 py-2 border rounded"><option value="percentage">نسبة</option><option value="fixed">مبلغ</option></select>
        <Input type="number" placeholder="القيمة" value={form.value} onChange={(e)=> setForm({ ...form, value: parseInt(e.target.value)||0 })} />
        <Input placeholder="من" value={form.validFrom||''} onChange={(e)=> setForm({ ...form, validFrom: e.target.value })} />
        <Input placeholder="إلى" value={form.validTo||''} onChange={(e)=> setForm({ ...form, validTo: e.target.value })} />
        <Button onClick={save}>حفظ</Button>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        {codes.map(c=> (
          <div key={c.id} className="p-3 rounded border flex items-center justify-between text-sm">
            <div>{c.code} • {c.type} • {c.value} • {c.validFrom||'-'} → {c.validTo||'-'}</div>
            <div className="flex items-center gap-2">
              <label className="inline-flex items-center gap-2 text-xs"><input type="checkbox" checked={c.enabled} onChange={(e)=> update({ ordersDiscounts: { ...(settings.ordersDiscounts||{}), codes: codes.map(x=> x.id===c.id? { ...x, enabled: e.target.checked }: x) } })} /> تفعيل</label>
      ...
      <div className="p-3 rounded border">
        <div className="font-medium arabic-safe mb-2">خصم العملاء الجدد</div>
        <div className="flex items-center gap-2">
          <label className="inline-flex items-center gap-2 text-sm arabic-safe"><input type="checkbox" checked={!!settings.ordersDiscounts?.defaultNewCustomer?.enabled} onChange={(e)=> update({ ordersDiscounts: { ...(settings.ordersDiscounts||{}), defaultNewCustomer: { ...(settings.ordersDiscounts?.defaultNewCustomer||{ type:'percentage', value:0 }), enabled: e.target.checked } } })} /> تفعيل</label>
          <select value={settings.ordersDiscounts?.defaultNewCustomer?.type || 'percentage'} onChange={(e)=> update({ ordersDiscounts: { ...(settings.ordersDiscounts||{}), defaultNewCustomer: { ...(settings.ordersDiscounts?.defaultNewCustomer||{ enabled:true, value:0 }), type: e.target.value as any } } })} className="px-3 py-2 border rounded"><option value="percentage">نسبة</option><option value="fixed">مبلغ</option></select>
          <NumericInput value={String(settings.ordersDiscounts?.defaultNewCustomer?.value || 0)} onChange={(v)=> update({ ordersDiscounts: { ...(settings.ordersDiscounts||{}), defaultNewCustomer: { ...(settings.ordersDiscounts?.defaultNewCustomer||{ enabled:true, type:'percentage' }), value: Math.max(0, Math.round(Number(v)||0)) } } })} />
        </div>
      </div>
    </div>
  );
...
const WarrantyByCategoryEditor: React.FC = () => {
  const { settings, update } = useSettings();
  const [list, setList] = useState(settings.ordersWarrantyByCategory || []);
  const [form, setForm] = useState<{ id: string; category: string; days: number }>({ id: '', category: '', days: 0 });

  useEffect(()=> setList(settings.ordersWarrantyByCategory || []), [settings.ordersWarrantyByCategory]);

  const save = () => {
    if (!form.category.trim()) return;
    const id = form.id && form.id.trim()!=='' ? form.id : 'wc_' + Date.now();
    const item = { ...form, id, days: Math.max(0, Math.round(Number(form.days)||0)) };
    const next = list.some(x=> x.id===id) ? list.map(x=> x.id===id? item: x) : [item, ...list];
    setList(next);
    update({ ordersWarrantyByCategory: next });
    setForm({ id:'', category:'', days:0 });
  };
  ...
  return (
    <div className="space-y-2">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
        <Input placeholder="فئة المنتج" value={form.category} onChange={(e)=> setForm({ ...form, category: e.target.value })} />
        <Input type="number" placeholder="أيام" value={form.days} onChange={(e)=> setForm({ ...form, days: parseInt(e.target.value)||0 })} />
        <Button onClick={save}>حفظ</Button>
      </div>
      <div className="space-y-2">
        {list.map(x=> (
          <div key={x.id} className="p-2 rounded border flex items-center justify-between text-xs">
            <div>{x.category} • {x.days} يوم</div>
            <div className="flex gap-2">
              <Button size="sm" variant="outline" onClick={()=> setForm(x)}>تعديل</Button>
              <Button size="sm" variant="destructive" onClick={()=> remove(x.id)}>حذف</Button>
            </div>
          </div>
        ))}
      </div>
