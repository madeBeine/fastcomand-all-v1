...
const FinancePage: React.FC<{ type: 'revenue' | 'expense'; title?: string }> = ({ type, title }) => {
  ...
  const [paymentMethods, setPaymentMethods] = useLocalStorageState<string[]>(`finance-settings-${type}-methods`, ['نقد', 'حوالة', 'بطاقة']);
  const [displayCurrency, setDisplayCurrency] = useLocalStorageState<string>(`finance-settings-${type}-currency`, 'MRU');
  const [allowMultiCurrency, setAllowMultiCurrency] = useLocalStorageState<boolean>(`finance-settings-${type}-multi`, type==='revenue');
  const [reportFreq, setReportFreq] = useLocalStorageState<'weekly'|'monthly'>(`finance-settings-${type}-reportfreq`, 'monthly');
  const [exportCsv, setExportCsv] = useLocalStorageState<boolean>(`finance-settings-${type}-export-csv`, true);
  const [exportPdf, setExportPdf] = useLocalStorageState<boolean>(`finance-settings-${type}-export-pdf`, true);
  const [budgets, setBudgets] = useLocalStorageState<Record<string, number>>(`finance-settings-${type}-budgets`, {});
  const [budgetAlerts, setBudgetAlerts] = useLocalStorageState<boolean>(`finance-settings-${type}-budget-alerts`, true);
  ...
  const handleAdd = async (e?: React.FormEvent) => {
    e?.preventDefault();
    if (!description.trim() || !amount) return alert('الرجاء إ��خال الوصف والمبلغ');
      ...
      <div className="max-w-6xl mx-auto">
        <header className="mb-4">
          <div className="flex items-start justify-between gap-3 mb-3">
            <div className="flex-1">
              <h1 className="text-xl sm:text-2xl font-bold arabic-safe">{title || (type === 'revenue' ? 'صفحة الإيرادات' : 'صفحة المصاريف')}</h1>
              <p className="text-xs sm:text-sm text-muted-foreground arabic-safe">سجل وإدارة العمليات المالية</p>
            </div>
            <button onClick={()=> setSettingsOpen(true)} className="p-2 rounded-lg bg-gray-100 hover:bg-gray-200 touch-manipulation" aria-label="إعدادات الصفحة">
        ...
        <Sheet open={settingsOpen} onOpenChange={setSettingsOpen}>
          <SheetContent side="right" className="w-full sm:max-w-md max-h-screen overflow-y-auto">
            <SheetHeader>
              <SheetTitle>إعدادات صفحة {title || (type==='revenue' ? 'الإيرادات' : 'المصاريف')}</SheetTitle>
            </SheetHeader>
            <div className="mt-4">
              <Tabs defaultValue={type==='revenue' ? 'categories' : 'budget'}>
                <TabsList className="flex gap-2 p-2 bg-gray-100 rounded-lg overflow-x-auto">
                  <TabsTrigger value="categories">{type==='revenue' ? 'تصنيف الإيرادات' : 'تصنيف المصاريف'}</TabsTrigger>
                  {type==='expense' && <TabsTrigger value="budget">الميزانية</TabsTrigger>}
                ...
                <TabsContent value="categories" className="mt-4 space-y-3">
                  <div className="p-3 rounded-xl border bg-card">
                    <div className="font-semibold mb-2">{type==='revenue' ? 'مصادر الإيرادات' : 'تصنيفات المصاريف'}</div>
                ...
                {type==='expense' && (
                  <TabsContent value="budget" className="mt-4 space-y-3">
                    <div className="p-3 rounded-xl border bg-card">
                      <div className="font-semibold mb-2">الميزانية لكل تصنيف</div>
                      <div className="space-y-2">
                        {categories.map(c => (
                          <div key={c} className="flex items-center gap-2">
                            <div className="min-w-[120px] text-sm">{c}</div>
                            <input type="number" value={budgets[c] || 0} onChange={(e)=> setBudgets({ ...budgets, [c]: Number(e.target.value||0) })} className="px-3 py-2 border rounded w-full" />
                          </div>
                        ))}
                      </div>
                      <label className="flex items-center gap-2 text-sm mt-3"><input type="checkbox" checked={budgetAlerts} onChange={(e)=> setBudgetAlerts(e.target.checked)} /> تنبيه عند تجاوز السقف</label>
                    </div>
                  </TabsContent>
                )}
                ...
                <TabsContent value="currency" className="mt-4 space-y-3">
                  <div className="p-3 rounded-xl border bg-card space-y-2">
                    <div className="font-semibold">العملة الافتراضية</div>
                    <select className="w-full px-3 py-2 border rounded" value={displayCurrency} onChange={(e)=> setDisplayCurrency(e.target.value)}>
                      <option value="MRU">MRU (أوقية)</option>
                      <option value="USD">USD (دولار)</option>
                      <option value="EUR">EUR (يورو)</option>
                    </select>
                    <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={allowMultiCurrency} onChange={(e)=> setAllowMultiCurrency(e.target.checked)} /> السماح بعملات متعددة مع تحويل تلقائي</label>
                  </div>
                </TabsContent>
                ...
                <TabsContent value="reports" className="mt-4 space-y-3">
                  <div className="p-3 rounded-xl border bg-card space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        <div className="text-sm mb-1">التردد</div>
                        <select value={reportFreq} onChange={(e)=> setReportFreq(e.target.value as any)} className="w-full px-3 py-2 border rounded"><option value="monthly">شهري</option><option value="weekly">أسبوعي</option></select>
                      </div>
                      <div className="space-y-1">
                        <div className="text-sm">تصدير</div>
                        <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={exportCsv} onChange={(e)=> setExportCsv(e.target.checked)} /> CSV/Excel</label>
                        <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={exportPdf} onChange={(e)=> setExportPdf(e.target.checked)} /> PDF</label>
                      </div>
                    </div>
                    <div className="font-semibold">طرق الدفع</div>
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      {['نقد','حوالة','بطاقة','شيك'].map(m => (
                        <label key={m} className="flex items-center gap-2">
                          <input type="checkbox" checked={paymentMethods.includes(m)} onChange={(e)=> {
                            setPaymentMethods(prev => e.target.checked ? Array.from(new Set([...prev, m])) : prev.filter(x=> x!==m));
                          }} /> {m}
                        </label>
                      ))}
                    </div>
        ...
        <button onClick={() => setShowAddModal(true)} className="fixed bottom-16 right-4 z-50 bg-brand-blue p-3 rounded-full text-white shadow-lg sm:hidden touch-manipulation">
          <Plus size={20} />
        </button>
        ...
        <div className="mb-4 space-y-3">
          <Input placeholder="بحث..." value={search} onChange={(e)=> setSearch(e.target.value)} className="w-full h-12" />

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            <select value={sortBy} onChange={(e)=> setSortBy(e.target.value as any)} className="px-3 py-3 border rounded-lg text-sm">
            ...
            <select value={sortDir} onChange={(e)=> setSortDir(e.target.value as any)} className="px-3 py-3 border rounded-lg text-sm">
              <option value="desc">تنازلي</option>
              <option value="asc">تصاعدي</option>
            </select>

            <select value={categoryFilter} onChange={(e)=> setCategoryFilter(e.target.value)} className="px-3 py-3 border rounded-lg text-sm">
            ...
            <Card key={entry.id} className="p-4 touch-manipulation">
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <Badge className={`px-2 py-1 text-xs ${type === 'revenue' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{type === 'revenue' ? 'إيراد' : 'مصروف'}</Badge>
                      {entry.automatic && <span className="text-xs px-2 py-1 rounded bg-gray-100 text-gray-700">تلقائي</span>}
                    </div>
                    <div className="font-medium arabic-safe text-base">{entry.description}</div>
                    <div className="text-sm text-muted-foreground arabic-safe">{entry.category} • {fmtDate(new Date(entry.date))}</div>
                    <div className="text-xs text-muted-foreground arabic-safe">بواسطة {entry.createdBy}</div>
                  </div>

                  <div className="text-right">
                    <div className="font-semibold text-lg numeric">{formatPrice(entry.amount)}</div>
                    <div className="text-xs text-muted-foreground">{new Date(entry.createdAt).toLocaleString('ar-MR')}</div>
                  </div>
                </div>

                {entry.notes && <div className="text-sm arabic-safe">{entry.notes}</div>}
                ...
                <div className="flex items-center gap-2 pt-2 border-t">
                  <Button variant="outline" size="sm" onClick={() => handleStartEdit(entry)} className="flex items-center gap-1 flex-1">
                    <Edit size={14}/> تعديل
                  </Button>
                  <Button variant="ghost" size="sm" onClick={() => handleDelete(entry.id)} className="text-red-600 flex items-center gap-1 flex-1">
                    <Trash size={14}/> حذف
                  </Button>
                </div>
              </div>
...
const EntryEditor: React.FC<{ entry: FinanceEntry; categories: string[]; onSave: (e: FinanceEntry)=> void; onCancel: ()=> void; onDelete: (id: string)=> void }> = ({ entry, categories, onSave, onCancel, onDelete }) => {
  ...
  const downloadAllAttachments = (attachments: {name: string; url: string}[]) => {
    attachments.forEach(att => downloadDataUrl(att.url, att.name));
  };
  ...
  return (
    <div>
      <div className="space-y-3">
            ...
            {attachments.map((a, idx) => (
              <div key={a.url} className="relative w-20 h-20 border rounded overflow-hidden">
                ...
                <div className="absolute top-1 left-1 flex gap-1">
                  ...
                  <button onClick={() => window.open(a.url, '_blank')} className="bg-white/80 rounded p-1 text-xs">عرض</button>
                </div>
              </div>
            ))}
        ...
        <div>
          <label className="text-sm">أضف مرفقات (متبق�� {Math.max(0, 3 - attachments.length)})</label>
          <input type="file" accept="image/*,application/pdf" multiple onChange={(e)=> handleFiles(e.target.files)} className="mt-1" />
